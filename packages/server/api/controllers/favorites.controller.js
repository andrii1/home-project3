...
const getFavoritesByUserId = async (token) => {
  ...
  try {
    const favorites = await knex('questions')
      .select('questions.*', 'favorites.id as favoritesID')
      .leftJoin('favorites', function () {
        this.on('questions.id', '=', 'favorites.question_id');
      })
...
const deleteFavorites = async (token, favoritesId) => {
  ...
  try {
    const deletedFav = await knex('favorites')
      .where({ question_id: favoritesId, user_id: user.id })
      .del();
    if (deletedFav === 0) {
